...
console.log('
 🚀
 Iniciando nosso Editor Criativo!'); 
// ============================================ 
// 
�
�
...
console.log('
 🎛
 Configurações iniciais definidas:', config); 
...
�
 APLICAR CONFIGURAÇÕES DIRETAS 
    ...
    console.log('
 ✨
 Visualização atualizada com sucesso!'); 
} 
...
texto.trim().split(/\s+/).length; 
    ...
    console.log(`
 📊
 Estatísticas: ${caracteres} caracteres, ${palavras} 
palavras`); 
} 
...
function aplicarConfiguracaoSalva() { 
    ...
    console.log('
 ⚙
 Configuração salva aplicada:', config); 
} 
...
texto.substring(fim); 
    ...
    mostrarMensagem(`
 😊
 Emoji ${emojiAleatorio} adicionado!`); 
} 
 
// 
�
�
 FUNÇÃO 9: Limpar tudo 
function limparTudo() { 
...
✨
 ')) { 
        ...
        mostrarMensagem('
 🧹
 Tudo limpo! Pronto para criar algo novo!'); 
    } 
} 
...
// 
⌨
 EVENTO 1: Digitação em tempo real 
...
selectFonte.addEventListener('change', function() { 
    ...
    console.log('
 🔤
 Fonte alterada para:', config.fonte); 
    atualizarVisualizacao(); 
}); 
...
tamanhoFonte.addEventListener('input', function() { 
    ...
    console.log('
 📏
 Tamanho da fonte:', config.tamanho + 'px'); 
    atualizarVisualizacao(); 
}); 
...
btnSombra.addEventListener('click', function() { 
    ...
    console.log('
 💫
 Sombra:', config.estilos.sombra ? 'ATIVADA' : 
'DESATIVADA'); 
    atualizarVisualizacao(); 
}); 
...
mostrarMensagem('
 ✨
 Bem-vindo ao Editor Criativo! Comece a 
digitar...'); 
}, 1000); 
console.log('
 ✅
 Editor criativo totalmente carregado e pronto!'); 